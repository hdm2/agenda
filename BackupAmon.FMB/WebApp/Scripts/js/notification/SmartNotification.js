...
var icones = ["fa fa-bullhorn", "fa fa-info", "glyphicon glyphicon-ok", "glyphicon glyphicon-warning-sign", "glyphicon glyphicon-fire"];
...
(function ($) {
    $.SmartMessageBox = function (settings, callback) {
        ...
        var stiloCor = "";
        if (settings.tipo != undefined) {
            settings.icon = obterIcone(settings.tipo);
        } else {
            stiloCor = "style='color: " + settings.color + ";'";
        }


        Content = "<div class='MessageBoxContainer animated fadeIn fast icon-notification " + settings.tipo + "' id='Msg" + SmartMSGboxCount +
            "'>";
        Content += "<i class='text-" + settings.tipo + " icon-not " + settings.icon + "' " + stiloCor + "> </i>"
        Content += "<div class='MessageBoxMiddle'>";
        Content += "<span class='MsgTitle text-" + settings.tipo + "' " + stiloCor + ">" + settings.title + "</span>";
