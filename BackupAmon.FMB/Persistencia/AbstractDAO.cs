...
namespace Amon.Persistencia
{
    public abstract class SimplesAbstractDAO<E> : SimplesAbstractDAO, ISimplesDao where E : AbstractEntidade
    {
        ...
        public virtual E incluir(E entidade)
        {
            ...
            if (!String.IsNullOrEmpty(obterCampoAutoIncrementado()) && param.ContainsKey(obterCampoAutoIncrementado()))
            {
            ...
            sql.Append(obterTabela());
            sql.Append(entidade.instrucaoParaInsert(obterCampoAutoIncrementado()));
            try
            {
                if (!String.IsNullOrEmpty(obterCampoAutoIncrementado()))
                {
                    ...
                    PropertyInfo pInfo = entidade.GetType().GetProperty(obterCampoAutoIncrementado(), new Type[] { });
