 	...
 	<div id="top" class="row">
		...
		<div class="col-sm-6">
			<div class="input-group h2">
				<input name="data[search]" class="form-control" id="search" type="text" placeholder="Pesquisar por Telefone">
				<span class="input-group-btn">
					<button id="btn-search" class="btn btn-primary" type="submit" data-toggle="modal" >
		...
		<div class="col-sm-3">
			<a href="add" class="btn btn-primary pull-right h2">Novo Contato</a>
		</div>
	</div> <!-- /#top -->
 	<hr />
			...
			<tbody>
			 <!-- ko foreach: contatos --> 
				<tr>
					...
					<td data-bind="text: $parent.nome"></td>
					<td data-bind="text: $parent.telefone"></td>
					<td class="actions">
...
<div class="modal fade" id="search-modal" tabindex="-1" role="dialog" aria-labelledby="modalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Fechar"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="modalLabel">Pesquisar Contato</h4>
      </div>
      <div id="search-content"class="modal-body">
      </div>
      <div class="modal-footer">
     	<button type="button" class="btn btn-default" data-dismiss="modal">Fechar</button>
        <button id="btn-delete" type="button" class="btn btn-primary">Excluir</button>
      </div>
    </div>
  </div>
</div>
 ...
 <script src="https://cdnjs.cloudflare.com/ajax/libs/knockout/3.4.2/knockout-min.js"></script>
  ...
  contatoSel.id="11";
  contatoSel.nome="nn";
  contatoSel.telefone="tt";
  console.log("contatoSel inicializado");
  console.log(contatoSel);
  function contatoViewModel() {
	    var self = this;
        self.contatos=ko.observableArray([]);
        self.hasFetchedContatos = ko.observable(false);    
	    self.getContatos = function () {
	        $.ajax({
	            type: 'GET',   
	            url: 'findall',
	            ...
	            success: function(data) {
	            	console.log("dados");
	            	console.log(data);
                     self.contatos.push(data.data);
	            	 console.log("contatos");
	            	 console.log(self.contatos);
	            	 self.hasFetchedContato=ko.observable(true);
                },
	...
	$(document).ready(function () {
        ...
        $("#btn-search").click(function(){
        	contatoSearch=$('#search').val();
            dynamic_text="telefone: "+contatoSearch;
            
	        $.ajax({
	            type: 'GET',   
	            url: 'findbytelefone/'+$('#search').val(),
	            contentType: "application/json",
	            dataType: "json",
                data: { "telefone": $('#search').val() },
	            success: function(data) {
	            	console.log("dados findbytelefone");
	            	console.log(data);
	            	if (data.status=="Done") {
	            		contatoSel.id=data.data.id;
	            		contatoSel.nome=data.data.nome;
	            		contatoSel.telefone=data.data.telefone;
	                    $('#search-content').text("O telefone " + $('#search').val()+" foi encontrado Nome="+data.data.nome+"(id="+data.data.id+")");
	                }
                    else {
    	                $('#search-content').text("O telefone " + $('#search').val()+" não foi encontrado");
                    }                    
	                $('#search-modal').modal('show');
                },
	            error:function(jq, st, error){
	            	console.log("findbytelefone error");
	            	console.log(st);
	            	console.log(error);
	                $('#search-content').text("Foi encontrado um erro na requisição");
	                $('#search-modal').modal('show');
	            }
	        });
        });
        $("#btn-delete").click(function() {
	        ...
	        $.ajax({
	            type: 'GET',   
	            url: 'deletecontato/'+contatoSel.id,
	            contentType: "application/json",
	            dataType: "json",
                data: { "id": contatoSel.id },
	            success: function(data) {
	            	console.log("dados deletecontato");
	            	console.log(data);
                },
	            error:function(jq, st, error){
	            	console.log("deletecontato error");
	            	console.log(st);
	            	console.log(error);
	            }
	        });
        	alert("Excluiu o contato id="+contatoSel.id+" Nome: "+contatoSel.nome+" Telefone: "+contatoSel.telefone);
        	 $('#search-modal').modal('toggle');
        });
		
		cvm=new contatoViewModel();
